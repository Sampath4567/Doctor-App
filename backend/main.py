...
def _is_patient(user: models.User) -> bool:
    """Return True if user has patient role (handles both enum and string from DB)."""
    r = getattr(user, "role", None)
    if r is None:
        return False
    if hasattr(r, "value"):
        return r.value == "patient"
    return r == "patient" or r == models.UserRole.patient
