...
def _part_of_day_range(part_of_day: str) -> Optional[Tuple[str, str]]:
    """Map a coarse part of day (or common synonyms) to a concrete time range (start, end) in HH:MM."""
    part = part_of_day.strip().lower()
    # Morning: ~8am–12pm
    if part in ("morning", "am", "morn", "mornings"):
        return "08:00", "12:00"
    # Afternoon: ~12pm–5pm
    if part in ("afternoon", "noon", "midday", "afternoons", "pm"):
        return "12:00", "17:00"
    # Evening: ~5pm–9pm
    if part in ("evening", "night", "eve", "evenings", "late afternoon"):
        return "17:00", "21:00"
    return None
...
def resolve_slot_from_intent(db: Session, user: models.User, intent: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _is_patient = (
        _role is not None
        and (getattr(_role, "value", _role) == "patient" or _role == models.UserRole.patient)
    )
    if not _is_patient:
        return {
            "status": "error",
    ...
    if time_str:
        ...
        if not rng:
            msg = (
                ...
                "Please give a specific time (e.g. 10:00) or say morning, afternoon, or evening (noon/midday = afternoon, night = evening)."
            )
            return {"status": "need_clarification", "message": msg, "slot": None}
        time_range = rng
    ...
    if not time_str and not time_range:
        ...
        if not parts:
            ...
            listing = " | ".join(parts)
            example_doc = candidate_doctors[0].user.full_name
            example_time = slots[0].start_time
            msg = (
                f"Available slots on {target_date} are: {listing}. "
                "Please reply with the exact doctor name, date, and time you prefer. "
                f'For example: "Book Dr. {example_doc} on {target_date} at {example_time}".'
            )
